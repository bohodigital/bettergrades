...
function makeSpec(brief) {
  ...
  // Keep internal visual-editor controls in the handoff record. Public
  // accessibility text is composed only from reviewed instructional fields.
  const interactive = brief.recommended_renderer === INTERACTIVE; const longDescription = `${plain(brief.long_description)} ${plain(brief.learning_purpose)}`; const caption = `${brief.title}. ${brief.learning_purpose}`;
