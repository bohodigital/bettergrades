...
import { calculusUnitSectionGuidance, getCalculusUnitCollection } from "../lib/calculus/calculus-units-index.mjs";
...
function TextbookOrientation({ page }: { page: CalculusUnitPublicPage }) {
  const guidance = calculusUnitSectionGuidance[page.page.sectionId] ?? calculusUnitSectionGuidance.review;
...
function UnitMap({ page }: { page: CalculusUnitPublicPage }) {
  ...
  return <section className="limits-unit-map calculus-unit-map" aria-label={`${page.unit.shortTitle} textbook map`}>
    <header className="limits-map-intro"><div><p className="eyebrow">Core textbook</p><h2>The complete Unit 2A path</h2></div><p>Start with meaning, build the differentiation toolkit in sequence, and use reviews to make rule selection independent of page labels.</p></header>
    <section className="calculus-prerequisites"><div><p className="eyebrow">What this unit teaches</p><h3>Turn local change into a dependable derivative toolkit.</h3><p>Connect limits, tangent slopes, formulas, graphs, tables, and units; then differentiate powers, products, quotients, special functions, compositions, implicit equations, inverses, and variable exponents.</p></div><div><p className="eyebrow">Prerequisites</p><h3>Algebra, functions, and Unit 1 limits.</h3><p>You should be comfortable with factoring, exponents, function notation, slopes, and finite limits. Use the diagnostic when you are unsure.</p></div></section>
    <div className="limits-chapter-map">{sections.map((section) => <section className="limits-chapter" id={`unit-${section.id}`} key={section.id}><header><div><span>Section</span><h3>{section.title}</h3></div><p>{calculusUnitSectionGuidance[section.id]?.lens}</p></header><aside className="limits-reading-lens limits-map-lens" aria-label={`${section.title} reading lens`}><span>Reading lens</span><p>{calculusUnitSectionGuidance[section.id]?.mentalModel}</p></aside><ol>{core.filter((route) => route.sectionId === section.id).map((route) => <li key={route.path}><a href={route.path}><span>{String(route.coreSequenceIndex).padStart(2, "0")}</span><b>{route.title}</b><small>{labels[route.pageType] ?? route.pageType.replaceAll("-", " ")}</small></a></li>)}</ol></section>)}</div>
    <div className="limits-map-stats" aria-label="Unit progress and pacing"><span><b>{page.unit.coreRouteCount}</b> core pages</span><span><b>{sections.length}</b> sections</span><span><b>9–12</b> study hours</span><span><b>{page.unit.problemCount}</b> interactive checks</span><span><b>{page.unit.visualCount}</b> BVLP visuals</span></div>
    ...
    <div className="limits-support-grid">{explorations.map((route) => <a href={route.path} key={route.path}><span>Advanced exploration</span><b>{route.title}</b><small>{route.description}</small></a>)}{deepDives.map(([href, title]) => <a href={href} key={href}><span>Focused article</span><b>{title}</b><small>A concise in-depth exploration connected to the main unit.</small></a>)}</div>
    <section className="limits-exam-key-callout"><div><p className="eyebrow">Next in the textbook</p><h2>Unit 2B: Applications of Derivatives</h2><p>The source is ready, but the link remains release-gated until the Unit 2A production postmortem is complete.</p></div><span className="tag">Coming after verified 2A release</span></section>
  </section>;
}
