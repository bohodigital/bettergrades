...
function AssessmentDirectory({ eyebrow, title, intro }: { eyebrow: string; title: string; intro: string }) { return <Shell><section className="page-hero section-pad"><Eyebrow>{eyebrow}</Eyebrow><h1>{title}</h1><p>{intro}</p></section><section className="activity-list section-pad">{assessments.map((item) => <Link href={item.path} className="activity-row" key={item.id}><span>Practice</span><div><Eyebrow>{practiceLabels[item.kind]} · Mathematics · Calculus</Eyebrow><h2>{item.title}</h2><p>{item.description}</p><span className="tag">Explained feedback</span><span className="tag">Device-local progress</span></div><b>Start →</b></Link>)}{limitsUnitPracticeRoutes.map((item) => <Link href={item.path} className="activity-row" key={item.path}><span>Unit 1</span><div><Eyebrow>{item.pageType.replaceAll("-", " ")} · Calculus I · Limits</Eyebrow><h2>{item.h1}</h2><p>{item.description}</p><span className="tag">Full worked solutions</span><span className="tag">No account required</span></div><b>Start →</b></Link>)}{calculusUnitPracticeRoutes.map((item) => { const unitLabel = item.unitId === "calc-1-unit-2b-derivative-applications" ? "Unit 2B" : item.unitId === "calc-1-unit-3a-integral-foundations-techniques" ? "Unit 3A" : item.unitId === "calc-1-unit-3b-integration-applications" ? "Unit 3B" : "Unit 2A"; return <Link href={item.path} className="activity-row" key={item.path}><span>{unitLabel}</span><div><Eyebrow>{item.pageType.replaceAll("-", " ")} · Calculus I · {unitLabel}</Eyebrow><h2>{item.title}</h2><p>{item.description}</p><span className="tag">Attempt-and-reveal</span><span className="tag">No account required</span></div><b>Start →</b></Link>; })}</section></Shell>; }
