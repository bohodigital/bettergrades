...
export function GlossaryHubPage({ terms }: { terms: readonly MathGlossaryTerm[] }) {
  return <><section className="glossary-hero section-pad"><p className="eyebrow">Glossaries</p><h1>One language layer.<br /><em>Every subject.</em></h1><p>Definitions, notation, symbols, and conventions live in subject-specific registries so the library can grow without turning into one undifferentiated list.</p></section><section className="glossary-hub-list section-pad"><a href="/glossary/math/"><span className="product-mark">∑</span><div><p className="eyebrow">Active glossary</p><h2>Mathematics</h2><p>{terms.length} visual definitions across symbols, foundations, algebra, and calculus.</p><small>Alphabetized terms · Sitewide popovers · Notation conventions</small></div><b>Open glossary →</b></a><div className="glossary-future-slot"><span>Next subject</span><p>Future subject registries plug into this index without changing the mathematics data model.</p></div></section></>;
}
