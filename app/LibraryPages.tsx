...
export function CourseHubContent({ domainSlug }: { domainSlug: string }) {
  ...
  return (
    <>
      ...
      {domainSlug === "calculus" && <section className="calculus-resource-paths section-pad" aria-labelledby="calculus-two-paths"><div><p className="eyebrow">Two useful paths</p><h2 id="calculus-two-paths">Follow the course—or focus your practice.</h2><p>The sequential course teaches each idea in order. The practice and reference library gives you printable worksheets, finals, formula sheets, worked problems, and visual guides without displacing that sequence.</p></div><div><a href="/subjects/math/calculus/limits-continuity/"><span>Path 1 · Learn in order</span><b>Start the complete calculus course</b><small>Lessons, concept checks, unit reviews, and exams →</small></a><a href="/subjects/math/calculus/worksheets/"><span>Path 2 · Practice and reference</span><b>Open the calculus resource library</b><small>Worksheets, finals, formula sheets, visuals, and worked examples →</small></a></div></section>}
