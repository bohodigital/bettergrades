...
- `page-terms.ts` selects the small contextual vocabulary set shown on instructional detail routes. Article terms are inferred from the article and topic registry with a short explicit topic profile as the stable starting point. Search, library and category browsing, glossary indexes, policy pages, and general hubs intentionally return no page terms.
...
4. Add the term ID to a topic or fixed-route profile only when it is genuinely central to an instructional detail page. The Vocab bar should remain a concise orientation layer, not a dump of every word in the article or navigation chrome on browsing pages.
