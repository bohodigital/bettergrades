...
function validateSource(value) {
  ...
  if (normalized.length > 240) {
    return { result: error("Keep this one smaller", "This release accepts expressions up to 240 characters.") };
  }
  return { normalized };
}
...
function parseWithChecks(ce, source) {
  ...
  if (JSON.stringify(expression.json).length > 5000) {
...
async function prepareExpression(value) {
...
function engineLimitResult() {
  return error("The calculator did not start", "Your expression is still here. Check your connection and press the button again; the calculator runs only in this browser.");
}

export async function simplifyAlgebraExpression(value) {
  try {
    const prepared = await prepareExpression(value);
    if (prepared.result) return prepared.result;
    const simplified = prepared.expression.simplify();
    return {
      status: "simplified",
      title: "Here is the simplified form",
      message: "The symbolic work happened in this browser. Check domain restrictions separately when denominators or even roots are involved.",
...
export async function compareAlgebraExpressions(leftValue, rightValue) {
  try {
    const left = await prepareExpression(leftValue);
...
export async function evaluateAlgebraExpression(value, assignmentSource) {
  try {
    ...
    return {
      ...
      message: "The substitution and arithmetic happened locally in this browser.",
      latex: exact.latex,
