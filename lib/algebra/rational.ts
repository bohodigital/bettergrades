...
export const rationalArticles = [
  ...
  algebraArticle({
    ...
    example: { heading: "Build a shared denominator", prompt: "Simplify 2/(x − 1) − 3/(x + 2).", steps: [[String.raw`D=(x-1)(x+2),\quad x\ne1,-2`, "Choose the LCD and restrictions."], [String.raw`\frac{2(x+2)}D-\frac{3(x-1)}D`, "Supply each missing factor."], [String.raw`\frac{2x+4-3x+3}{D}`, "Distribute the subtraction through the second numerator."], [String.raw`\frac{7-x}{(x-1)(x+2)}`, "Combine like terms."]], result: String.raw`\boxed{\frac{7-x}{(x-1)(x+2)},\quad x\ne1,-2}` },
  ...
  algebraArticle({
    ...
    course: "Algebra II", difficulty: "Intermediate", minutes: 11,
    formula: String.raw`\frac{A(x)}{B(x)}=\frac{C(x)}{D(x)}`, immediate: { label: "Workflow", text: "Restrictions first, denominator clearing second, solution check last. Clearing fractions does not erase the original domain." },
    sections: [
      ["Restrictions come before algebra", "Set every original denominator factor unequal to zero. This exclusion list becomes the final filter for candidate solutions.", "If a later operation produces an excluded value, report that it is extraneous rather than quietly keeping it.", String.raw`B(x)D(x)\ne0`],
      ["The LCD must multiply every term", "Multiplying both sides by the least common denominator clears fractions because each denominator divides it. Terms without visible denominators are multiplied too.", "Write the multiplier beside each term before canceling to avoid skipping a constant or distributing incompletely.", String.raw`L\left(\frac A p+q\right)=\frac{LA}p+Lq`],
