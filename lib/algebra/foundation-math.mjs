...
function replaceSimpleFractions(tex) {
  let previous = "";
  let current = tex;
  const productParenthesized = /(?<![\\A-Za-z])([A-Za-z0-9.]+)\(([^()]+)\)\/([A-Za-z0-9.]+)/g;
  const parenthesized = /\(([^()]+)\)\/([A-Za-z0-9.]+)/g;
  const simple = /(?<![\\A-Za-z])([A-Za-z0-9.]+)\/([A-Za-z0-9.]+)(?![A-Za-z])/g;
  while (current !== previous) {
    previous = current;
    current = current
      .replace(productParenthesized, String.raw`\frac{$1($2)}{$3}`)
