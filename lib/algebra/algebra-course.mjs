...
export { algebraCourseRoutes, supersededAlgebraPaths } from "./algebra-course-index.mjs";
export { algebraCourseSearchRecords } from "./algebra-course-search.mjs";
...
export function getPublicAlgebraCoursePage(path) {
  ...
  return {
    ...
    lesson: {
      ...page.lesson,
      figures: page.lesson.figures
        .map((figure) => ({
          ...figure,
          visual: visualsById.get(`algebra-${figure.id.toLowerCase().replaceAll(".", "-")}`),
        }))
        .filter((figure) => figure.visual?.isFunctionGraph),
    },
  };
}
