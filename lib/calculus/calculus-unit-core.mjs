const semanticEnvironmentTypes = Object.freeze({
  ...
  bgvisualreading: "visual-reading",
});
...
function visualReference(state, environment, inner = "", details = {}) {
  ...
  return {
    ...
    visualId,
    title: details.title || "Visual study",
    ...((details.text || caption) ? { text: cleanText(details.text || caption) } : {}),
  };
}

function bgVisualDetails(inner) {
  let cursor = 0;
  const values = [];
  while (values.length < 3) {
    while (/\s/.test(inner[cursor] ?? "")) cursor += 1;
    const argument = readBraced(inner, cursor);
    if (!argument) throw new Error("A bgvisual environment requires id, title, and description arguments.");
    values.push(argument.value);
    cursor = argument.end;
  }
  return {
    declaredId: cleanText(values[0]),
    title: cleanText(values[1]),
    text: cleanText(values[2]),
...
function parseSegment(source, state) {
  ...
  while (cursor < source.length) {
    ...
    if (environment === "bgvisual") {
      const details = bgVisualDetails(inner);
      const expectedId = state.visualIds[state.visualIndex];
      if (expectedId && expectedId !== details.declaredId) {
        throw new Error(`Visual order mismatch: expected ${expectedId}, received ${details.declaredId}.`);
      }
      nodes.push(visualReference(state, environment, details.body, details));
    } else if (environment === "bgexercises") nodes.push(...exerciseNodes(inner, state));
