...
function commandArgument(source, command) {
  ...
  while (cursor < source.length) {
    ...
    if (/[A-Za-z]/.test(source[after] ?? "")) { cursor = after; continue; }
    let braceStart = after;
    while (/\s/.test(source[braceStart] ?? "")) braceStart += 1;
    const braced = readBraced(source, braceStart);
    if (braced) return cleanText(braced.value);
    cursor = after;
  }
  return null;
}
...
function tableBody(inner, environment) {
  ...
  if (body.startsWith("[")) {
    const optionEnd = body.indexOf("]");
    if (optionEnd >= 0) body = body.slice(optionEnd + 1).trimStart();
  }
...
function tableRows(inner, environment) {
  const body = tableBody(inner, environment).replace(/\\(?:toprule|midrule|bottomrule|hline|endhead|endfoot)\b/g, " ");
...
function structuredDisplayNode(tex) {
  ...
  if (tableEnvironments.has(environment)) return { type: "table", environment, rows: tableRows(inner, environment) };
  if (graphEnvironments.has(environment)) return graphSpecificationNode(environment, inner);
  return { type: "math", tex: normalizeMathEnvironment(String(tex).trim()) };
}

function graphSpecificationNode(environment, inner) {
  const caption = commandArgument(inner, "caption");
  const graphMatchers = [
    [/Secant lines through/i, "secant-tangent"],
    [/undefined at .*nearby outputs approach/i, "removable-hole"],
    [/nearby parabola determines the limit/i, "limit-versus-value"],
    [/one-sided limits produce a jump/i, "jump-discontinuity"],
    [/oscillates increasingly rapidly/i, "rapid-oscillation"],
    [/oscillating function is trapped between/i, "squeeze-bounds"],
    [/unit circle.*inner triangle/i, "unit-circle-squeeze"],
    [/removable hole at .*approaches .*both sides/i, "sine-over-x"],
    [/odd power changes sign.*even power remains positive/i, "vertical-asymptotes"],
    [/rational function approaches its horizontal asymptote/i, "horizontal-asymptote"],
    [/four principal discontinuity types/i, "discontinuity-gallery"],
    [/endpoint signs guarantee at least one root/i, "ivt-root"],
    [/band around .*corresponding .*window/i, "epsilon-delta-window"],
  ];
  const graphId = caption ? graphMatchers.find(([pattern]) => pattern.test(caption))?.[1] : undefined;
  return {
    type: "graph-specification",
    title: graphId ? "Read the graph" : "Graph reading guide",
    ...(graphId ? { graphId } : {}),
    text: caption || (environment === "figure"
      ? "Read the graph together with the surrounding explanation; focus on the nearby behavior named in the lesson."
      : "A graph accompanies this explanation in the printable edition."),
  };
}
...
function parseSegment(source) {
  ...
  while (cursor < source.length) {
    ...
    if (environment === "bgexercises") nodes.push(...exerciseNodes(inner));
    ...
    else if (tableEnvironments.has(environment)) nodes.push({ type: "table", environment, rows: tableRows(inner, environment) });
    else if (graphEnvironments.has(environment)) nodes.push(graphSpecificationNode(environment, inner));
      ...
      if (type === "graph-specification") nodes.push({ type, title: "Interactive graph exploration", children: parseSegment(inner) });
      else nodes.push({ type, ...(title ? { title } : {}), ...(type === "quick-check" && title ? { checkId: title } : {}), children: parseSegment(inner) });
...
export function normalizeLimitChoice(value) {
...
export function numericLimitValue(value) {
...
export function normalizeLimitExpression(value) {
  return String(value).trim().replace(/\\(?:varepsilon|epsilon)|ε/g, "epsilon")
    .replace(/\\frac\{([^{}]+)\}\{([^{}]+)\}/g, "($1)/($2)")
    .replace(/\\min\s*\{([^{}]+)\}/g, "min($1)").replace(/[{}\s]/g, "").replace(/\(([^()]+)\)/g, "$1").replace(/[−–—]/g, "-");
}

export function isSufficientEpsilonDelta(check, answer) {
  if (check.id !== "epsilon-flow-01") return false;
  const [numerator, denominator] = normalizeLimitExpression(answer).split("/");
