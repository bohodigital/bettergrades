...
const mathEnvironments = new Set(["align", "align*", "aligned", "array", "cases", "gather", "gather*", "groupplot", "longtable", "tabular", "tabularx"]);
...
function parseSegment(source) {
  ...
  while (cursor < source.length) {
    ...
    if (match[0] === "\\[") {
      ...
      nodes.push({ type: "math", tex: source.slice(special.lastIndex, end).trim() });
    ...
    else if (mathEnvironments.has(environment)) nodes.push({ type: "math", tex: `\\begin{${environment}}${inner}\\end{${environment}}` });
    else if (["figure", "tikzpicture", "axis"].includes(environment)) nodes.push({ type: "graph-specification", title: environmentTitle(inner), text: cleanText(inner), children: parseSegment(inner) });
...
export function parseLimitsUnitPage(source) {
  const objective = String(source).match(/\\lessonobjective\{([\s\S]*?)\}\s*(?=\n)/);
  const nodes = parseSegment(source);
  if (objective) nodes.unshift({ type: "concept", title: "Learning objectives", children: paragraphNodes(objective[1]) });
  return nodes;
}
...
export async function compareLimitAnswer(check, answer) {
  ...
  if (check.answerType === "integer" || check.answerType === "rational") {
    ...
    if (actual === expected) correct = true;
