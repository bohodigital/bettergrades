...
function publicNode(node, context) {
  ...
  return {
    ...node,
    ...(node.children ? { children: node.children.map((child) => publicNode(child, context)) } : {}),
  };
}
...
export function getPublicCalculusUnitPage(path) {
  ...
  const assessmentSet = payload.assessmentSets.assessments.find((assessment) => assessment.route === route.slug);
  return {
    unit: collection.unit,
    route,
    page: {
      nodes: page.nodes.map((node) => publicNode(node, context)),
...
export function getCalculusUnitReveal(unitId, routeId, revealId) {
  ...
  const context = { route, revealIndex: 0 };
  return nodes.map((node) => publicNode(node, context));
}
