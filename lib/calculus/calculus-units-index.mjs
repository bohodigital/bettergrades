...
import unit4b from "../../content/calculus/units/unit-4b/routes.public.json" with { type: "json" };
import { limitsUnitRoutes } from "./limits-unit-index.mjs";

const unitCollections = [unit2a, unit2b, unit3a, unit3b, unit4a, unit4b];
...
const unit4bSectionGuidance = Object.freeze({
  orientation: { lens: "Connect every power-series calculation to its center, radius, interval, and approximation purpose.", mentalModel: "A power series is both an infinite numerical series at each input and a function on the inputs where it converges.", decision: "Identify the center and coefficient pattern before choosing a convergence or Taylor method.", commonTrap: "Manipulating a series without recording where the resulting identity is valid.", checkpoint: "Can you name the center, variable, coefficient formula, and current convergence question?" },
  convergence: { lens: "Find the radius from interior behavior, then test each boundary point as a separate numerical series.", mentalModel: "Distance from the center organizes the automatic interior and exterior behavior; endpoints remain independent decisions.", decision: "Use a ratio or root argument for the radius, convert it to an interval, and test both endpoints explicitly.", commonTrap: "Including or excluding both endpoints from the radius calculation alone.", checkpoint: "Can you justify the radius and each endpoint with separate evidence?" },
  operations: { lens: "Treat algebra, differentiation, and integration as coefficient transformations with a preserved or rechecked interval.", mentalModel: "Inside a power series' convergence interval it behaves like a polynomial limit, so familiar operations become structured index shifts.", decision: "Write the source identity and validity interval before transforming coefficients or powers.", commonTrap: "Losing an index, constant of integration, or endpoint condition during a formal manipulation.", checkpoint: "Can you reverse the operation and recover the source series?" },
  "taylor-construction": { lens: "Match value and derivatives at one center, then separate the polynomial approximation from the infinite-series convergence claim.", mentalModel: "Taylor coefficients encode local derivative data as a polynomial of increasing degree.", decision: "Choose the center, compute the derivative pattern, divide by factorials, and state whether you need a polynomial or an infinite series.", commonTrap: "Assuming every smooth-looking function equals its Taylor series everywhere.", checkpoint: "Can you verify the first coefficients directly from derivatives at the center?" },
  "series-library": { lens: "Build new expansions from a small verified library using explicit substitutions, products, derivatives, or integrals.", mentalModel: "A standard series is a reusable identity with a domain, not a formula fragment detached from convergence.", decision: "Name the source series, apply one transformation at a time, and transform its interval alongside it.", commonTrap: "Recognizing a pattern but omitting the substitution's effect on the interval.", checkpoint: "Can you reconstruct the expansion and its domain without memorizing the final line?" },
  approximation: { lens: "Pair every Taylor approximation with a degree, center, target input, and certified remainder bound.", mentalModel: "The polynomial supplies the estimate; the remainder theorem supplies the trust boundary.", decision: "Choose a tractable center and degree, bound the needed derivative on the whole interval, then compare the bound with the required tolerance.", commonTrap: "Evaluating the next term without checking that the theorem's hypotheses make it a valid error bound.", checkpoint: "Can you state exactly why the reported digits are certified?" },
  advanced: { lens: "Use coefficient recurrences and moving error envelopes to preview later analysis without weakening the core claims.", mentalModel: "Series methods can solve equations and expose different notions of convergence, but each conclusion depends on its stated domain.", decision: "Track the coefficient rule or error supremum explicitly and distinguish pointwise observations from uniform guarantees.", commonTrap: "Generalizing a finite graph or pointwise limit into a stronger convergence statement.", checkpoint: "Can you identify which quantity must be bounded uniformly?" },
  review: { lens: "Mixed work tests recognition: classify the task before selecting a convergence, construction, transformation, or error method.", mentalModel: "A complete solution preserves the series identity, interval, endpoint logic, and approximation claim as one chain.", decision: "Write the objective and conditions first, solve independently, then use one answer reveal to repair the earliest incorrect decision.", commonTrap: "Checking algebra while leaving the convergence domain or error certification unstated.", checkpoint: "Can you reproduce the method and domain without the key?" },
});
...
export function getCalculusUnitSectionGuidance(unitId, sectionId) {
  ...
  if (unitId === "calc-2-unit-4b-power-taylor-series") return unit4bSectionGuidance[sectionId] ?? unit4bSectionGuidance.review;
