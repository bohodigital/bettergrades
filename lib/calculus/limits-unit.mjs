...
export { LIMITS_UNIT_PREFIX, unitPayload as limitsUnitPayload };

export const limitsUnitRoutes = unitPayload.routes.map((route) => ({
  ...route,
  metadataTitle: route.title.replace(/\s*\|\s*BetterGrades$/i, ""),
}));

const pagesBySourceFile = new Map(unitPayload.pages.map((page) => [page.sourceFile, page]));
...
export function getLimitsUnitPage(path) {
  ...
  const page = pagesBySourceFile.get(route.sourceFile);
