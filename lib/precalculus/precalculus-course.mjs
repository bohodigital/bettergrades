...
const assessmentsById = new Map(course.assessments.map((assessment) => [assessment.id, assessment]));
const promptsById = new Map(course.lessons.flatMap((lesson) => [lesson.checkpoint, ...lesson.practice]).map((prompt) => [prompt.id, prompt]));
...
export function getPublicPrecalculusCoursePage(path) {
  ...
  const sourceAssessment = route.assessmentId ? assessmentsById.get(route.assessmentId) ?? null : null;
  const assessment = sourceAssessment ? {
    ...sourceAssessment,
    items: sourceAssessment.items.map((reference) => ({ ...promptsById.get(reference.id), ...reference })),
  } : null;
  ...
  const breadcrumbs = [
    ...
    ...(assessment ? [{ name: assessment.title, path: assessment.path }] : []),
  ];
  return {
    route,
    unit,
